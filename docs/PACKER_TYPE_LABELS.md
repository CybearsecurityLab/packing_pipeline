...
Generated by `ops/qemu/build_packer_type_document.py` from `manifest/empirical_types_*.yaml`. Do not hand-edit.

**107 corpus definitions** · 101 typed · 6 untyped.  
*(manifest rows below: 108 typed / 6 unresolved across 114 keys; a condition named differently in packer_corpus.yaml and worklist.json appears under both, so that denominator is larger than the corpus.)*
...
| telock | 0.98 | None | **TYPE_VI-B** | max-observed |

## Pipeline-unresolved (6)
